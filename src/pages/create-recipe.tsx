...
import AddTagModal from "../components/CreateRecipe/AddTagModel"; // Corrected import
...
export default function CreateRecipe(): ReactElement {
    ...
    const createRecipe = async (e: FormEvent): Promise<void> => {
        e.preventDefault();
        ...
        try {
            ...
            const recipeResponse = await axios.post<Recipe>("/api/recipes", {
                public: isPublic,
                title,
                description,
                category,
            ...
            toast.success(`Created new recipe: ${recipeResponse.data.title}`);
            await router.push(`/recipes/${recipeResponse.data.id}`);
        } catch (error) {
            const axiosError = error as AxiosError;
            toast.error(
                axiosError.response?.data
                    ? `Failed to create recipe: ${axiosError.response.data as string}`
    ...
    const editor = useEditor({
        ...
        onUpdate: e => {
            setInstructions(e.editor.getHTML());
        },
    });

    useEffect(() => {
        if (status === "unauthenticated") {
            router.push("/sign-in").catch(() => toast.error("Failed to redirect when not logged in."));
        }
    }, [status, router]);
    ...
    return (
        <>
            <section className="mx-auto max-w-2xl">
                ...
                <form className="mx-auto" onSubmit={createRecipe}>
                    <div className="grid gap-4 sm:grid-cols-2 sm:gap-6 mb-4">
                        <div className="sm:col-span-2">
                            <div className="flex items-center me-4">
                                ...
                                />
                                <label htmlFor="public" className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                    Recipe is Public
                                </label>
                            </div>
                        </div>
                        ...
                        <div>
                            <label htmlFor="difficulty" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Difficulty
                            </label>
                            <select
                                id="difficulty"
                                ...
                                value={difficulty}
                                onChange={e => {
                                    setDifficulty(e.target.value as RecipeDifficulty);
                                }}
                                required
                            >
                                <option value="" disabled>
                        ...
                        <div className="sm:col-span-2">
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {tools.length ? (
                                        tools.map(tool => (
                                            <tr key={tool.id} className="bg-white border-y dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <tbody>
                                    {ingredients.length ? (
                                        ingredients.map(ingredient => (
                                            <tr key={ingredient.ingredientId} className="bg-white border-y dark:bg-gray-800 dark:border-gray-700">
                                                ...
                                                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white flex justify-between">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() =>
                                                            setIngredients(ingredients.filter(t => t.ingredient.id !== ingredient.ingredient.id))
                                                        }
                                                    >
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div className="sm:col-span-2">
                            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    {tags.length ? (
                                        tags.map(tag => (
                                            <tr key={tag.id} className="bg-white border-y dark:bg-gray-800 dark:border-gray-700">
                                                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white flex justify-between">
                                        ...
                                        <div className="flex flex-wrap items-center divide-gray-200 sm:divide-x sm:rtl:divide-x-reverse dark:divide-gray-600">
                                            <div className="flex items-center space-x-1 rtl:space-x-reverse sm:pe-4">
                                                ...
                                                <button
                                                    ...
                                                    } p-2 text-gray-500 rounded cursor-pointer hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400`}
                        ...
                        <div className="sm:col-span-2">
                            <label htmlFor="timeRequired" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Time Required
                            </label>
                            <input
                                type="text"
                        ...
                        <div className="sm:col-span-2">
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                {imageFile && (
                                    <span className="text-sm text-gray-500 dark:text-gray-400">{imageFile.name}</span>
                                )}
                            </div>
                        </div>
                    </div>
                    <button
                        ...
                        className="disabled:cursor-not-allowed disabled:bg-slate-500 disabled:hover:bg-slate-500 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    >
                        Create
                    </button>
                </form>
            ...
            <AddToolModal
                addTool={(tool: Tool) => {
                    if (!tools.some(t => t.id === tool.id)) setTools([...tools, tool]);
                }}
                currentTools={tools}
            ...
            <AddIngredientModal
                addIngredient={(ingredient: Ingredient) => {
                    if (!ingredients.some(t => t.ingredient.id === ingredient.id))
                        setIngredients([
                            ...ingredients,
                            {
