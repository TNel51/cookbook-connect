...
export default function MyRecipes(): ReactElement {
    ...
    const loadMoreRecipes = useCallback((): void => {
        ...
        axios.get<{total: number; recipes: RecipeEntity[];}>(`/api/recipes?mine=true&take=10&skip=${recipes.length}${categoryFilter !== "Any" ? `&category=${categoryFilter}` : ""}${difficultyFilter !== "Any" ? `&difficulty=${difficultyFilter}` : ""}`)
            .then(res => {
    ...
    }, [loadingMore, recipes, totalRecipes, categoryFilter, difficultyFilter]);
